...
public class ZkConnector implements Watcher {
    ...
    protected String getData(String path, Watcher watch, Stat stat) throws KeeperException, InterruptedException {
    	byte[] byteData = zooKeeper.getData(path, watch, stat);
    	return new String(byteData);
    }
    
    protected KeeperException.Code setData(String path, String data) {
    	try {
            byte[] byteData = null;
            if(data != null) {
                byteData = data.getBytes();
            }
            zooKeeper.setData(path, byteData, -1);
        } catch(KeeperException e) {
            return e.code();
        } catch(Exception e) {
            return KeeperException.Code.SYSTEMERROR;
